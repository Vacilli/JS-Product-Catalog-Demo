...
const closeIcon = document.querySelector(
  'div.search__box > i.fa-magnifying-glass'
)
  ...
  fetch(
    ...
    .then((data) => {
      ...
      productCatalog.forEach((element, index) => {
        ...
        index >= 0 && index <= pageSize - 1
          ? renderProduct([element][0].fields, ` active`)
          : renderProduct([element][0].fields)
      })

      paginationConstructor(
        roundNum((getPageCount([...slides]) + pageSize - 1) / pageSize)
      )
...
typeSelect.addEventListener('change', (e) => {
  ...
  productCatalog.forEach((element, index) => {
    ...
    if ([element][0].fields.Type == e.target.value)
      renderProduct([element][0].fields, ` active`)
  })

  // if number of slides is greater than pagesize then render navigation
  if (getPageCount([...slides]) >= pageSize)
    paginationConstructor(
      roundNum((getPageCount([...slides]) + pageSize - 1) / pageSize)
    )
})
